...
>
        ...
        <header class="mb-8 p-6 bg-white rounded-xl shadow-md border border-slate-200">
            <div>
                ...
                <p class="mt-1 text-slate-500">
                    <span x-text="`Terdapat ${productionPlans.length} rencana produksi yang perlu dikerjakan.`"></span>
                </p>
            </div>
        </header>
                ...
                <div class="bg-white rounded-xl shadow-lg border border-slate-200 flex flex-col overflow-hidden transition-transform hover:-translate-y-1">
                    <div class="p-5 flex-grow">
                        <div class="flex justify-between items-start mb-3">
                            <div>
                                <p class="font-bold text-xl text-slate-800" x-text="`#${plan.id}`"></p>
                        ...
                        <ul class="text-sm space-y-1 mb-4">
                            <template x-for="product in plan.products.slice(0, 2)" :key="product.sku">
                             ...
                             <li x-show="plan.products.length > 2">
                                <span class="text-slate-400 text-xs" x-text="`+${plan.products.length - 2} produk lainnya...`"></span>
                            </li>
                        </ul>
                    </div>
                    ...
                    <div class="bg-slate-50 border-t p-4 flex gap-3">
                        <!-- Tombol Update Status, hanya muncul jika BELUM selesai -->
                        <button x-show="plan.status_produksi !== 'Selesai'" @click="openUpdateModal(plan.id)" class="w-full bg-sky-600 text-white font-semibold py-2 px-4 rounded-lg hover:bg-sky-700 transition-colors text-sm flex items-center justify-center gap-2">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" /><path fill-rule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clip-rule="evenodd" /></svg>
                            Update Status
                        </button>
                        
                        <!-- Tombol Lihat Detail, hanya muncul jika SUDAH selesai -->
                        <button x-show="plan.status_produksi === 'Selesai'" @click="openUpdateModal(plan.id)" class="w-full bg-slate-600 text-white font-semibold py-2 px-4 rounded-lg hover:bg-slate-700 transition-colors text-sm flex items-center justify-center gap-2">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path d="M10 12a2 2 0 100-4 2 2 0 000 4z" /><path fill-rule="evenodd" d="M.458 10C1.732 5.943 5.522 3 10 3s8.268 2.943 9.542 7c-1.274 4.057-5.022 7-9.542 7S1.732 14.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z" clip-rule="evenodd" /></svg>
                            Lihat Detail
                        </button>

                        <!-- Tombol Kelola Laporan, hanya muncul jika SUDAH selesai -->
                        <button @click="openReportModal(plan.id)" x-show="plan.status_produksi === 'Selesai'" class="w-full bg-green-600 text-white font-semibold py-2 px-4 rounded-lg hover:bg-green-700 transition-colors text-sm flex items-center justify-center gap-2">
    ...
    <div x-show="isModalOpen" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50" x-transition.opacity style="display: none;">
        <div @click.away="isModalOpen = false" class="bg-white rounded-xl shadow-2xl w-full max-w-4xl transform" x-transition.scale>
            <div class="p-6 border-b">
                <h3 class="text-xl font-bold text-slate-800" x-text="`Update Status Produksi #${selectedPlan?.id}`"></h3>
            </div>
            <div class="p-6 max-h-[70vh] overflow-y-auto">
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <!-- Kolom Kiri: Catatan & Riwayat -->
                    <div class="space-y-6">
                        <div>
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <ul class="divide-y divide-slate-200 bg-slate-50 border rounded-lg p-2 shadow-inner max-h-48 overflow-y-auto">
                                <template x-if="selectedPlan">
                                     <template x-for="product in selectedPlan.products" :key="product.sku">
            ...
            <div class="p-6 bg-slate-50 border-t flex justify-end gap-3 rounded-b-xl">
                ...
                <button x-show="selectedPlan?.status_produksi !== 'Selesai'" @click="updateStatus" class="bg-sky-600 text-white font-bold py-2 px-6 rounded-lg hover:bg-sky-700 transition-colors">Simpan Perubahan</button>
            </div>
        </div>
    </div>
    ...
    <div x-show="isReportModalOpen" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50" x-transition.opacity style="display: none;">
        <div @click.away="isReportModalOpen = false" class="bg-white rounded-xl shadow-2xl w-full max-w-3xl transform" x-transition.scale>
            <div class="p-6 border-b">
                <h3 class="text-xl font-bold text-slate-800" x-text="`Laporan Aktual Produksi #${selectedPlan?.id}`"></h3>
                 <p class="text-sm text-slate-500">Input jumlah produk yang berhasil diproduksi dan yang ditolak.</p>
            </div>
            <div class="p-6 max-h-[60vh] overflow-y-auto">
                <div class="space-y-4">
                    <template x-if="selectedPlan">
                        <template x-for="(product, index) in reportInputs" :key="product.sku">
                             <div class="grid grid-cols-1 md:grid-cols-5 gap-4 items-center bg-slate-50 p-3 rounded-lg border">
                                ...
                                <div class="md:col-span-1">
                                    <label :for="'hasil-' + product.sku" class="block text-xs font-medium text-slate-600 mb-1">Berhasil</label>
                                    <input :id="'hasil-' + product.sku" type="number" min="0" x-model.number="reportInputs[index].hasil_produksi" class="w-full rounded-md border-slate-300 shadow-sm focus:border-green-500 focus:ring-green-500 text-sm">
                                </div>
                                <div class="md:col-span-1">
                                     <label :for="'reject-' + product.sku" class="block text-xs font-medium text-slate-600 mb-1">Reject</label>
                                     <input :id="'reject-' + product.sku" type="number" min="0" x-model.number="reportInputs[index].reject_produksi" class="w-full rounded-md border-slate-300 shadow-sm focus:border-red-500 focus:ring-red-500 text-sm">
                                </div>
                                <div class="md:col-span-1 text-center md:text-right">
                                     <p class="text-xs text-slate-500 font-medium">Total Input</p>
                                     <p class="font-bold text-slate-800" x-text="(reportInputs[index].hasil_produksi + reportInputs[index].reject_produksi).toLocaleString('id-ID')"></p>
                                </div>
                            </div>
                        </template>
                    </template>
            ...
            <div class="p-6 bg-slate-50 border-t flex justify-end gap-3 rounded-b-xl">
                <button @click="isReportModalOpen = false" class="bg-slate-200 text-slate-700 font-semibold py-2 px-6 rounded-lg hover:bg-slate-300 transition-colors">Batal</button>
                <button @click="saveReport" class="bg-green-600 text-white font-bold py-2 px-6 rounded-lg hover:bg-green-700 transition-colors">Simpan Laporan</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    function productionStaff() {
        return {
            ...
            updateStatus() {
                if (!this.selectedPlanId || this.newStatus === this.selectedPlan.status_produksi) {
                ...
                if (planIndex > -1) {
                    // Update status
                    this.productionPlans[planIndex].status_produksi = this.newStatus;
